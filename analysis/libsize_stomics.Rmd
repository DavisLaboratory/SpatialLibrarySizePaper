---
title: 'STOmics Mouse Brain - Library size confounds biology in spatial transcriptomics'
...
```{r}
tx_detections = eh[['EH8231']]
#create and add DNB IDs
tx_detections = tx_detections |> 
  mutate(dnb_id = paste(x, y, sep = '_'))
head(tx_detections)
...
This workflow analyses the BGI STOmics data. The colour map used by the Allen Brain Atlas (ABA) is used to visualise annotated data points. This is stored in the _ABA_V3_ontology.csv_ file.
...
Since cell boundary detection is still a work in progress, we bin individual detections in to a hexagonal grid with 100 bins along each axis. The results are saved into a `SpatialExperiment` object where each column represents a bin and each row a gene.
...
tx_detections |> 
  ...
  annotate('text', y = 142, x = 4, label = '*', size = 6, vjust = 0.75) +
...
This analysis shows that the slope (rate of increase in transcript density with respect to cell density) is generally higher in the cortex (dark green) than in the white matter of the brain (pinks).
...
coef_df |> 
  ...
  labs(
    ...
    caption = '* 3 outlier removed for visualisation purposes'
  ) +
  lims(x = c(0.98, 1.2), y = c(0, 31)) +
  vissE::bhuvad_theme()
```
